...
size_t SafeWrite(int fd, const char* ptr, size_t size) {
	size_t retval = 0;
	while (size > 0) {
...
size_t SafeRead(int fd, char* ptr, size_t size) {
	size_t retval = 0;
	while (size > 0) {
...
void WriteBinary(int out, uint32_t number) {
  number = htonl(number);
  SafeWrite(out, reinterpret_cast<char*>(&number), sizeof(number));
}
void WriteBinary(int out, uint64_t number) {
  number = htonll(number);
  SafeWrite(out, reinterpret_cast<char*>(&number), sizeof(number));
}
void WriteBinary(int out, const char* str, uint32_t length) {
  WriteBinary(out, uint32_t(length));
  SafeWrite(out, str, length);
}
